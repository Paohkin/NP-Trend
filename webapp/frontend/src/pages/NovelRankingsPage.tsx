    ...
    <div className="p-4 lg:container mx-auto">
      <div className="mb-3">
        ...
        <p className="text-muted mb-0">
          ...
          <Link to="/data-collection-info" className="ms-2 subtle-link">
            (데이터 수집 방식)
          </Link>
        </p>
      </div>
            ...
            <thead>
              <tr>
                <th onClick={() => requestSort('Ranking')} className="cursor-pointer sortable-header text-center" style={{ fontSize: '0.85rem', width: '40px' }}>
                  <div className="d-flex align-items-center justify-content-center gap-1"><span>순위</span>{getSortIndicator('Ranking')}</div>
                </th>
                ...
                processedRankings.map((novel) => (
                  <tr key={novel.ID}>
                    ...
                    <td style={{ fontSize: '0.9rem' }}>
                      {novel.AuthorID ? (
                        <Link to={`/authors/${novel.AuthorID}`}>
                          {novel.AuthorName || '(작자 미상)'}
                        </Link>
                      ) : (
                        novel.AuthorName || '(작자 미상)'
                      )}
                    </td>
                    <td style={{ fontSize: '0.9rem' }}>{novel.Score}</td>
